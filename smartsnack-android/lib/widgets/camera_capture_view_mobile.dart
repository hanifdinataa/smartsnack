...
class _CameraCaptureViewState extends State<CameraCaptureView> {
  ...
  // Simpan args Map, bukan Uint8List, agar isolate dapat semua
  ...
  Future<void> _setup() async {
    ...
    try {
      ...
      // _initializing HARUS direset di finally, termasuk saat
  ...
  Future<void> _startPreviewStream() async {
    ...
    // Guard terhadap double-start stream. Tanpa ini, jika
    ...
    await controller.startImageStream((CameraImage image) {
      ...
      // Tidak lewatkan CameraImage langsung ke isolate (tidak serializable),
  ...
  void _startFrameWorker() {
    ...
    // Poll setiap 120ms (sebelumnya 350ms).
  ...
  Future<void> _drainPendingFrame() async {
    ...
    try {
      // Jalankan konversi YUV→JPEG di background isolate via compute().
  ...
  Future<void> _capture() async {
    ...
    // Stop worker DULU sebelum stopImageStream agar timer tick
    ...
    // Buang frame lama — tidak relevan lagi setelah tombol ditekan
    _pendingFrameArgs = null;
